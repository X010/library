...
public interface BookDao {
    ...
    @Insert("insert into t_book(name,isbn,release_type,price,release_time,author,book_size,cover,recomment_word," +
            "author_describe,book_describe) values(#{name},#{isbn},#{release_type},#{price},#{release_time},#{author}," +
            "#{book_size},#{cover},#{recomment_word},#{author_describe},#{book_describe})")
    public int addbook(Book book);

    @Select("select id,name,isbn,release_type,price,release_time,author,book_size,cover,recomment_word,author_describe," +
            "book_describe from t_book order by id desc")
    public List<Book> queryAllBook();

    @Select("select id,name,isbn,release_type,price,release_time,author,book_size,cover,recomment_word,author_describe," +
            "book_describe from t_book order by id desc limit #{start},#{size} ")
    ...
    @Select("select id,name,isbn,release_type,price,release_time,author,book_size,cover,recomment_word,author_describe," +
            "book_describe from t_book where name=#{name} order by id desc")
    public List<Book> queryBookByNanme(@Param(value = "name") String name);


    @Select("select id,name,isbn,release_type,price,release_time,author,book_size,cover,recomment_word,author_describe," +
            "book_describe from t_book where isbn=#{isbn} limit 1")
    public Book queryBookByIsbn(@Param(value = "isbn") String isbn);


    @Select("select id,name,isbn,release_type,price,release_time,author,book_size,cover,recomment_word,author_describe," +
            "book_describe from t_book where id=#{id} limit 1")
    ...
    @Update("update t_book set name=#{name},isbn=#{isbn},release_type=#{release_type},price=#{price},release_time=#{release_time}," +
            ...
            "book_describe=#{book_describe} where id=#{id}")
    public int updateBook(Book book);


    @Select("select id,name,isbn,release_type,price,release_time,author,book_size,cover,recomment_word,author_describe," +
            "book_describe from t_book where name like #{keyword} or author like #{keyword} or isbn=#{keyword} order by id desc")
    ...
    @Select("select id,name,isbn,release_type,price,release_time,author,book_size,cover,recomment_word,author_describe," +
            "book_describe from t_book where release_type=#{release_type}")
